...
# =============================
# STATE (même logique que ton autre bot)
# =============================
alerts_data: dict[int, dict] = {}  # message_id -> alert data
...
class DefenderSelect(discord.ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if added:
            ...
            msg = "Ces joueurs étaient déjà enregistrés."
...
class AlertsCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        bot.add_view(self.alert_view)  # EXACTEMENT comme ton autre bot
    ...
    def build_embed(self, data: dict) -> discord.Embed:
        ...
        if data["defenders"]:
            defenders = ", ".join(f"<@{d}>" for d in data["defenders"])
        else:
            defenders = "Aucun"
    ...
    # ---------- API UTILISÉE PAR LES REACTIONS / SELECT ----------
    ...
    async def send_alert(self, interaction, cooldown_key, role_id):
        ...
        embed = self.build_embed(data)
        msg = await channel.send(embed=embed, view=self.alert_view)
