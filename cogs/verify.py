...
class GuildSelect(discord.ui.Select):
    def __init__(self, options_src: List[Tuple[str, int]]):
        ...
        super().__init__(placeholder="🔽 Sélectionne ta guilde…", min_values=1, max_values=1, options=options)
    ...
    def chosen(self) -> Optional[Tuple[str, int]]:
        ...
        return (label, int(role_id_str)) if label is not None else None

class PseudoModal(discord.ui.Modal, title="📝 Pseudo en jeu"):
    pseudo = discord.ui.TextInput(label="Ton pseudo IG", placeholder="Ex: Kicard", required=True, max_length=64)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if guild is None:
            await interaction.response.send_message("Serveur introuvable.", ephemeral=True); return

        validation_channel = guild.get_channel(VALIDATION_CHANNEL_ID)
        if not isinstance(validation_channel, discord.TextChannel):
            await interaction.response.send_message("Canal de validation introuvable.", ephemeral=True); return
...
class WelcomeView(discord.ui.View):
    ...
    async def send_request(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.member.id:
            await interaction.response.send_message("Seul le joueur accueilli peut envoyer sa demande.", ephemeral=True); return
        chosen = self.guild_select.chosen()
        if not chosen:
            await interaction.response.send_message("Sélectionne d’abord ta **guilde**.", ephemeral=True); return
...
class ValidationButtons(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not isinstance(interaction.user, discord.Member) or not self._is_direction(interaction.user):
            await interaction.response.send_message("Action réservée à la **Direction**.", ephemeral=True); return
...
class ExtraRolesView(discord.ui.View):
    ...
    async def add_selected(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if member is None:
            await interaction.response.send_message("⚠️ Membre introuvable.", ephemeral=True); return
        ...
        await interaction.response.edit_message(
            content=f"✅ Rôles ajoutés : **{', '.join(applied) if applied else '—'}**\n🎉 La demande est terminée.",
            view=None,
        )
...
class VerifyCog(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        # Utiliser en priorité le salon d'arrivée configuré
        ch = guild.get_channel(ARRIVAL_CHANNEL_ID)
        if isinstance(ch, discord.TextChannel) and ch.permissions_for(guild.me).send_messages:
            channel = ch
        else:
            # fallback: salon système, puis premier salon où le bot peut parler
            if guild.system_channel and guild.system_channel.permissions_for(guild.me).send_messages:
                channel = guild.system_channel
            else:
                for ch2 in guild.text_channels:
                    if ch2.permissions_for(guild.me).send_messages:
                        channel = ch2
                        break
        if channel is None:
            return
